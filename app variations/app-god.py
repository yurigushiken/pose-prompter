...
@app.route('/')
def index():
    return render_template('index-god.html')
...
def handle_generate_image(json_data):
    ...
    while is_generating_continuously or not is_generating_continuously and json_data:
        try:
            with open('D:\consolidation\workflows\workflow_planetPaintCanny.json', 'r') as file:
        ...
        if not is_generating_continuously:
            break
        time.sleep(0.35)  # Adjust this value as needed for the rate of generation
